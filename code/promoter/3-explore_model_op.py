...
# %%
folder_root = pfa.get_output() / 'output_local'
...
# %%
scores = []
expression_prediction = pd.DataFrame(
    0.,
    index = transcriptome.obs.index,
    columns = transcriptome.var.index
)
expression_observed = pd.DataFrame(
    transcriptome.X.dense().cpu().detach().numpy(),
    index = transcriptome.obs.index,
    columns = transcriptome.var.index
)
...
def select_window(
    ...
    '''Select fragments non-overlapping the window'''
    return ~((fr_coordinates[:, 0] < window_end) & (fr_coordinates[:, 1] > window_start))
...
# %%
idx_gene_mapping = dict(zip(fragments.var['ix'], fragments.var.index))
peaks_bed = pybedtools.BedTool(folder_data_preproc / "atac_peaks.bed")
...
for split in tqdm.tqdm(splits):
    for gene_id in split.local_gene_idx.detach().cpu().numpy():
        ...
        promoter_bed = pybedtools.BedTool.from_dataframe(pd.DataFrame(promoter).T[["chr", "start", "end"]])
        peaks_in_promoter_df = promoter_bed.intersect(peaks_bed, ).to_dataframe()
        if peaks_in_promoter_df.shape[0] == 0:
            continue
#             peaks_in_promoter_df = pd.DataFrame(columns = ["start", "end"])
        else:
            peaks_in_promoter_df[["start", "end"]] = [
                [
                    (peak["start"] - promoter["tss"]) * promoter["strand"],
                    (peak["end"] - promoter["tss"]) * promoter["strand"]
                ][::promoter["strand"]]

                for _, peak in peaks_in_promoter_df.iterrows()
            ]
        
        fragments_oi_list = [
            select_window(split.fragments_coordinates, peak_start, peak_end).detach().cpu().numpy()
            for peak_start, peak_end in list(
                zip(peaks_in_promoter_df['start'],
                    peaks_in_promoter_df['end'])
            )
        ]
        if len(fragments_oi_list) > 1:
            fragments_oi = torch.Tensor(
                np.prod(np.vstack(fragments_oi_list), axis=0)
            ).bool().to("cuda")

        else:
            fragments_oi = torch.Tensor(fragments_oi_list[0]).bool().to("cuda")
        # calculate how much is retained overall
        perc_retained = fragments_oi.float().mean().detach().item()
...
scores_peaks_df = pd.DataFrame(scores_peaks)
...
# %%
mse = scores.groupby("phase")["mse"].mean()
mse_windows = scores_peaks_df.groupby(["phase"])["mse"].mean()#.unstack()
mse_dummy_windows = scores_peaks_df.groupby(["phase"])["mse_dummy"].mean()#.unstack()
fig, ax_mse = plt.subplots()
patch_train = ax_mse.plot(mse_windows.index, mse_windows["train"], color = "blue", label = "train")
ax_mse.plot(mse_dummy_windows.index, mse_dummy_windows["train"], color = "blue", alpha = 0.1)
ax_mse.axhline(mse["train"], dashes = (2, 2), color = "blue")
...
# %% [markdown]
# ## Masking windows and removing fragment lengths

# %%
# length
length_size = 1000 #length
length_cuts = list(np.arange(0, 1000, 10))
length_windows = [[cut0, cut1] for cut0, cut1 in zip(length_cuts, length_cuts[1:] + [9999999])]

# window
window_size = 100
padding_positive = 2000
padding_negative = 4000
window_cuts = np.arange(-padding_negative, padding_positive, step = window_size)

# %%
scores_lengths_windows = []
for split in tqdm.tqdm(splits):
    for window_idx, (window_start, window_end) in enumerate(zip(window_cuts[:-1], window_cuts[1:])):
        # take fragments within the window
        window_fragments_oi = select_window(split.fragments_coordinates, window_start, window_end)
        for len_window_idx, (len_window_start, len_window_end) in enumerate(length_windows):
            # take fragments within the length range
            fragment_lengths = (split.fragments_coordinates[:,1] - split.fragments_coordinates[:,0])
            len_fragments_oi = ~((fragment_lengths >= len_window_start) & (fragment_lengths < len_window_end))
            fragments_oi = window_fragments_oi & len_fragments_oi
            ...
            scores_lengths_windows.append({
                "mse": mse.detach().cpu().numpy(),
                "mse_dummy": mse_dummy.detach().cpu().numpy(),
                "phase": split.phase,
                "len_window_start": len_window_start,
                "window_mid": window_start + (window_end - window_start) / 2,
                "perc_retained": perc_retained,
                "genescores": genescores
            })
scores_len_window_df = pd.DataFrame(scores_lengths_windows)

# %%
scores_len_window_df.head()
...
# %%
mse = scores.groupby("phase")["mse"].mean()
mse_windows = scores_len_window_df.groupby(["window_mid",'len_window_start', "phase"])["mse"].mean().unstack()
mse_dummy_windows = scores_len_window_df.groupby(["window_mid", 'len_window_start',"phase"])["mse_dummy"].mean().unstack()
fig, ax_mse = plt.subplots()
patch_train = ax_mse.plot(mse_windows.index, mse_windows["train"], color = "blue", label = "train")
ax_mse.plot(mse_dummy_windows.index, mse_dummy_windows["train"], color = "blue", alpha = 0.1)
ax_mse.axhline(mse["train"], dashes = (2, 2), color = "blue")
