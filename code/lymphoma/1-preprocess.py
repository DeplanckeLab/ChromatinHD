...
# %%
# ! wget https://cf.10xgenomics.com/samples/cell-arc/2.0.0/lymph_node_lymphoma_14k/lymph_node_lymphoma_14k_filtered_feature_bc_matrix.h5 -O {folder_data_preproc}/filtered_feature_bc_matrix.h5

# %%
# !wget https://cf.10xgenomics.com/samples/cell-arc/2.0.0/lymph_node_lymphoma_14k/lymph_node_lymphoma_14k_atac_fragments.tsv.gz -O {folder_data_preproc}/atac_fragments.tsv.gz

# %%
# !wget https://cf.10xgenomics.com/samples/cell-arc/2.0.0/lymph_node_lymphoma_14k/lymph_node_lymphoma_14k_atac_fragments.tsv.gz.tbi -O {folder_data_preproc}/atac_fragments.tsv.gz.tbi

# %%
# !wget https://cf.10xgenomics.com/samples/cell-arc/2.0.0/lymph_node_lymphoma_14k/lymph_node_lymphoma_14k_atac_peaks.bed -O {folder_data_preproc}/atac_peaks.bed

# %%
# !cat {folder_data_preproc}/atac_peaks.bed | sed '/^#/d' > {folder_data_preproc}/peaks.tsv

# %%
# !wget https://cf.10xgenomics.com/samples/cell-arc/2.0.0/lymph_node_lymphoma_14k/lymph_node_lymphoma_14k_atac_peak_annotation.tsv -O {folder_data_preproc}/peak_annot.tsv
...
all_gene_ids = sorted(list(set(gff.loc[gff["chr"].isin(chromosomes)]["gene"]) & set(adata.var.index)))
...
# %%
sc.pl.umap(adata, color = transcriptome.gene_id(["PTPRC", "CD9",  "ANPEP", "FCGR3A"]))
...
# %%
promoters["tss"] = [gff_row["start"] if gff_row["strand"] == "+" else gff_row["end"] for _, gff_row in gff.loc[promoters.index].iterrows()]
promoters["strand"] = (gff["strand"] + "1").astype(int)
promoters["positive_strand"] = (promoters["strand"] == 1).astype(int)
promoters["negative_strand"] = (promoters["strand"] == -1).astype(int)
promoters["chr"] = gff.loc[promoters.index, "chr"]
...
# %%
sns.heatmap(gene_mse_windowpairs.loc["test"].loc[gene_id])
...
ax2 = ax.twinx()
ax2.plot(perc_retained_lengths.index, -perc_retained_lengths["train"], color = "red")
