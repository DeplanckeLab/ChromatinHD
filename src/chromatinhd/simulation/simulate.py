...
def sample_nb(_shape, total_count, logits):
    return scipy.stats.poisson(
        np.random.standard_gamma(total_count, _shape)
        / np.exp(-logits)
    ).rvs(_shape)
...
class Simulation():
    def __init__(self, n_cells = 1000, n_genes = 1000, window = (-5000, 5000)):
        ...
        self.cell_latent_space = np.hstack([
            latent_0,
            latent_0 ** 2,
            np.random.uniform(-1, 1, size = (self.n_cells, 1)) * (celltype == 0).astype(float),
            np.random.uniform(-1, 1, size = (self.n_cells, 1)) * (celltype == 1).astype(float),
        ...
        # lib
        sensitivity = 10.
        library_size = np.exp(np.random.normal(np.log(self.n_genes * sensitivityw), 0.2, self.n_cells))
        ...
        for gene_ix in tqdm.tqdm(range(self.n_genes)):
            ...
            n_fragments_gene = sample_nb(self.n_cells, *transform_nb2(n_fragments_gene_mean, 1.))
