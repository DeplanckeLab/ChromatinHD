...
def calculate_heights(unnormalized_heights, widths, min_bin_height = DEFAULT_MIN_BIN_HEIGHT):
...
def quadratic_spline(inputs,
    ...
    if inverse:
        c_ = c - inputs
        alpha = torch.clamp(
            (-b + torch.sqrt(b.pow(2) - 4*a*c_)) / (2*a),
            0,
            1
        )
        ...
        alpha = torch.clamp(
            (inputs - input_bin_locations) / input_bin_widths,
            0,
            1
        )
