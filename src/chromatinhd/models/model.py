class HybridModel:
        ...
        if autoextend:
            parameters.extend(
                [
                    ...
                    if (not contains(p, self.parameters_sparse())) and (not contains(p, parameters))
                ]
            )
        return parameters
    ...
    def parameters_sparse(self):
        ...
        return [
            parameter
            for module in self._modules.values()
            for parameter in (module.parameters_sparse() if hasattr(module, "parameters_sparse") else [])
        ]
