...
def initialize_mixture(coordinates, genemapping, n_genes, n_components=64):
    ...
    bincounts = torch.bincount(
        coordinates_binned + genemapping * len(bins), minlength=len(bins) * n_genes
    )
...
class Decoding(torch.nn.Module):
    def __init__(
        ...
        locs, logits = initialize_mixture(
            ...
            fragments.cut_local_gene_ix,
            fragments.n_genes,
            n_components=n_components,
        )
        self.mixture = Mixture(
            n_components, fragments.n_genes, loc_init=locs, logit_init=logits
        )
    ...
    def forward_(
        ...
        likelihood_mixture = self.track["likelihood_mixture"] = self.mixture.log_prob(
            cut_coordinates, mixture_delta, genes_oi, cut_local_gene_ix
        )

        # expression
        fragmentexpression = (
            self.rho_bias[genes_oi] * torch.exp(rho_change)
