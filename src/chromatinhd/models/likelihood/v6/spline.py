...
class DifferentialQuadraticSplineStack(torch.nn.Module):
    ...
    def transform_forward(
        ...
        bias = torch.tensor(
            0.0, device=cut_coordinates.device, dtype=cut_coordinates.dtype
        )
        ...
        for unnormalized_heights, unnormalized_widths, delta_heights in zip(
            ...
            unnormalized_heights = unnormalized_heights + delta_heights - bias
            heights = quadratic.calculate_heights(unnormalized_heights, widths)
            heights = heights  # * torch.exp(bias)
            ...
            logabsdet_ = torch.log(normalized_area).flatten()[
                cut_local_reflatentxgene_ix
            ] + math.log(2)
            ...
            heights = heights / normalized_area

            bin_left_cdf = quadratic.calculate_bin_left_cdf(heights, widths)

            cut_heights = torch.index_select(
                heights.flatten(0, 1), 0, cut_local_reflatentxgene_ix
            )
            cut_widths = torch.index_select(widths, 0, cut_local_gene_ix)
            cut_bin_left_cdf = torch.index_select(
                bin_left_cdf.flatten(0, 1), 0, cut_local_reflatentxgene_ix
            )
            cut_bin_locations = torch.index_select(bin_locations, 0, cut_local_gene_ix)
            ...
            # print(bias.flatten()[cut_local_reflatentxgene_ix])
            # logabsdet_ = logabsdet_ + bias.flatten()[cut_local_reflatentxgene_ix]
            outputs = outputs / normalized_area.flatten()[cut_local_reflatentxgene_ix]

            # import matplotlib.pyplot as plt

            # fig, ax = plt.subplots()
            # plt.plot(outputs.detach().cpu().numpy())
            # plt.plot(np.exp(logabsdet_.detach().cpu().numpy()))
            print(
                np.trapz(
                    np.exp(logabsdet_.detach().cpu().numpy()),
                    np.linspace(0, heights.shape[1], len(logabsdet_)),
                )
            )
    ...
    def transform_progressive(
        ...
        bias = torch.zeros_like(mixture_delta_reflatentxgene[..., [0]])
        ...
        for unnormalized_heights, unnormalized_widths, delta_heights in zip(
            ...
            unnormalized_heights_biased = unnormalized_heights
            heights_biased = quadratic.calculate_heights(
                unnormalized_heights_biased, widths
            )
            normalized_area_biased = (
                n_genes
                * torch.sum(  # TODO: can be made logaddexp, through cat of the logheights and -log(2) + log(widths)?
                    ((heights_biased[..., :-1] + heights_biased[..., 1:]) / 2) * widths,
                    dim=-1,
                    keepdim=True,
                )
            )
            normalized_area_biased = torch.exp(
                math.log(n_genes)
                + torch.logsumexp(
                    (
                        torch.logaddexp(
                            torch.log(heights_biased[..., :-1]),
                            torch.log(heights_biased[..., 1:]),
                        )
                        - math.log(2)
                    )
            ...
            area_diff = normalized_area_biased - normalized_area_biased.mean(
                1, keepdim=True
            )
            bias_diff = torch.exp(bias) - torch.exp(bias).mean(1, keepdim=True)
            bias2 = (area_diff / (bias_diff + 1e-8)) * bias

            unnormalized_heights = unnormalized_heights_biased - bias2
            heights = quadratic.calculate_heights(unnormalized_heights, widths)
            normalized_area = torch.sum(  # TODO: can be made logsumexp, through cat of the logheights and -log(2) + log(widths)?
                ((heights[..., :-1] + heights[..., 1:]) / 2) * widths,
                dim=-1,
                keepdim=True,
            )
            ...
            bias = torch.log(normalized_area) + bias + math.log(n_genes)
    ...
    def transform_progressive(
        ...
        for unnormalized_heights, unnormalized_widths, delta_heights in zip(
            ...
            #     unnormalized_heights_biased, widths
            # )
            # normalized_area_biased = n_genes * quadratic.calculate_area(
            #     heights_biased, widths
            # )
            ...
            heights = quadratic.calculate_heights(unnormalized_heights, widths)
            print(heights)
            lognormalized_area = quadratic.calculate_logarea(heights, widths)

            logabsdet_ = lognormalized_area.flatten()[
                cut_local_reflatentxgene_ix
            ] + math.log(n_genes)
            logabsdet = logabsdet + logabsdet_

            bias = lognormalized_area + bias + math.log(n_genes)

            heights = heights / torch.exp(lognormalized_area)
        ...
        # print(torch.bincount(cut_local_reflatentxgene_ix))
        return logabsdets
