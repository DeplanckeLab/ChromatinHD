...
class Decoder(torch.nn.Module):
    ...
    def forward(self, latent, genes_oi):
        ...
        # print("self.logit_weight", self.logit_weight.shape)
        logit_weight = self.logit_weight(genes_oi)
        # print("logit_weight", logit_weight.shape)

        # print("self.rho_weight", self.rho_weight.shape)
        rho_weight = self.rho_weight.get_full_weight()
        # print("rho_weight", rho_weight.shape)

        # print('nn', self.nn)
        nn_output = self.nn(latent)
        # print("nn_output", nn_output.shape)
...
class BaselineDecoder(torch.nn.Module):
    def __init__(self, n_latent, n_genes, n_output_components, n_layers=1, n_hidden_dimensions=32):
        print("---  BaselineDecoder.__init__()  ---")
        super().__init__()
...
class Decoding(torch.nn.Module, HybridModel):
    ...
    def __init__(
        ...
        if not baseline:
            self.decoder = Decoder(n_latent_dimensions, fragments.n_genes, n_delta_mixture_components, n_layers=decoder_n_layers)
        else:
            self.decoder = BaselineDecoder(n_latent_dimensions, fragments.n_genes, n_delta_mixture_components, n_layers=decoder_n_layers)
        ...
        if mixture_delta_p_scale_free:
            self.mixture_delta_p_scale = torch.nn.Parameter(torch.tensor(math.log(mixture_delta_p_scale), requires_grad=True))
        else:
    ...
    def forward_(self, local_cellxgene_ix, cut_coordinates, latent, genes_oi, cells_oi, cut_local_cellxgene_ix, cut_localcellxgene_ix, cut_local_gene_ix, n_cells, n_genes):
        ...
        # print("mixture_delta", mixture_delta.shape)
        # print("rho_delta", rho_delta.shape)
        ...
        likelihood_mixture = self.mixture.log_prob(cut_coordinates, genes_oi, cut_local_gene_ix, mixture_delta)

        # overall likelihood
        likelihood = (likelihood_mixture + torch.log(rho_cuts) + math.log(self.n_total_genes))
        ...
        # ELBO
        elbo = (-likelihood.sum() * likelihood_scale - mixture_delta_kl.sum() - rho_delta_kl.sum())
    ...
    def evaluate_pseudo(self, coordinates, latent=None, gene_oi=None, gene_ix=None):
        print("---  Decoding.evaluate_pseudo()  ---")
        
        device = coordinates.device # on which device is coordinates stored, other data needs to be on the same device
        ...
        if gene_ix is None:
            ...
            cut_localcellxgene_ix = (torch.ones_like(coordinates).to(torch.long) * gene_oi)
        else:
    ...
    def rank(self, window: np.ndarray, n_latent: int, device: torch.DeviceObjType = "cuda", how: str = "probs_diff_masked", prob_cutoff: float = None) -> np.ndarray:
        print("---  Decoding.rank()  ---")
        import pandas as pd
        ...
        for _, design_subset in tqdm.tqdm(design.groupby("batch")):
            ...
            prob = self.evaluate_pseudo(pseudocoordinates.to(device), latent=pseudolatent.to(device), gene_ix=gene_ix)
            probs.append(prob)
