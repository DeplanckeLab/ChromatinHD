...
import math
from peakfreeatac import splines
...
def prioritize(x, n, k = 2):
    ...
    dist = torch.distributions.Normal(0., scale = (1/(n*k*2)))
...
def initialize_from_previous(x, n, local_gene_ix, n_genes, transforms, device = "cuda"):
    ...
    for transform in transforms:
        q2 = transform.transform_forward(q2.flatten(), local_gene_ix = torch.repeat_interleave(torch.ones(n_genes, dtype = int) * n))[0].detach().reshape(q2.shape)
    ...
    for x2, local_gene_ix2 in tqdm.tqdm(zip(x.split(chunk_width, 0), local_gene_ix.split(chunk_width, 0)), total = math.ceil(x.shape[0]/chunk_width)):
        x2 = x2.to(device)
        local_gene_ix2 = local_gene_ix2.to(device)
        for transform in transforms:
            x2 = transform.transform_forward(x2, local_gene_ix2)[0].detach()
        
        digitized = torch.clamp(torch.searchsorted(q2[local_gene_ix2], x2.unsqueeze(-1), right = True).squeeze(-1), 0, n-1)
        bincount += torch.bincount((digitized + local_gene_ix2 * n), minlength = n*n_genes).reshape((n_genes, n))[:, 1:].cpu()
    ...
    aroundcounts = torch.nn.functional.pad(bincount / q2.diff(), (1, 0)) + torch.nn.functional.pad(bincount / q2.diff(), (0, 1))
...
class QuadraticSplineStack(torch.nn.Module):
    def __init__(self, x, nbins, local_gene_ix, n_genes):
        ...
        transforms = []
        for n in nbins:
            unnormalized_heights, unnormalized_widths = initialize_from_previous(x, n, local_gene_ix, n_genes, transforms = transforms)
...
class DifferentialQuadraticSplineStack(torch.nn.Module):
    def __init__(self, x, nbins, local_gene_ix, n_genes):
        ...
        split_deltas = []
        for n in nbins:
            unnormalized_heights_, unnormalized_widths_ = initialize_from_previous(x, n, local_gene_ix, n_genes, transforms = transforms)
        ...
        self.unnormalized_heights = EmbeddingTensor(n_genes, (unnormalized_heights.shape[1:]), sparse = True)
        self.unnormalized_heights.weight.data = unnormalized_heights

        self.unnormalized_widths = EmbeddingTensor(n_genes, (unnormalized_widths.shape[1:]), sparse = True)
        self.unnormalized_widths.weight.data = unnormalized_widths
    ...
    def parameters_sparse(self):
        return [
            self.unnormalized_heights.weight,
            self.unnormalized_widths.weight
        ]
