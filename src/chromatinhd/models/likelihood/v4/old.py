...
class Decoder(torch.nn.Module):
    def __init__(self, n_latent, n_genes, n_output_components, n_layers = 1, n_hidden_dimensions = 32, dropout_rate = 0.2):
        super().__init__()
        ...
        stdv = 1. / math.sqrt(self.rho_weight.weight.size(1))
    ...
    def parameters_sparse(self):
        return [
            self.logit_weight.weight,
            self.rho_weight.weight
        ]
...
class Decoding(torch.nn.Module):
    ...
    def __init__(self, fragments, cell_latent_space, nbins = (128, ), decoder_n_layers = 0, baseline = False, mixture_delta_p_scale_free = False, scale_likelihood = False, rho_delta_p_scale_free = False, mixture_delta_p_scale_dist = "normal", mixture_delta_p_scale = 1.):
        super().__init__()
        ...
        if mixture_delta_p_scale_free:
            self.mixture_delta_p_scale = torch.nn.Parameter(torch.log(torch.tensor(math.log(mixture_delta_p_scale), requires_grad=True)))
        else:
            self.register_buffer("mixture_delta_p_scale", torch.tensor(math.log(mixture_delta_p_scale)))
    ...
    def forward_(
        ...
        likelihood_mixture = self.track["likelihood_mixture"] = self.mixture.log_prob(cut_coordinates, genes_oi, cut_local_gene_ix, mixture_delta)
        ...
        if self.mixture_delta_p_scale_dist == "normal":
            mixture_delta_p = torch.distributions.Normal(0., torch.exp(self.mixture_delta_p_scale))
        else:
            mixture_delta_p = torch.distributions.Laplace(0., torch.exp(self.mixture_delta_p_scale))
        mixture_delta_kl = mixture_delta_p.log_prob(self.decoder.logit_weight(genes_oi))

        # expression
        rho_delta_p = torch.distributions.Normal(0., torch.exp(self.rho_delta_p_scale))
        rho_delta_kl = rho_delta_p.log_prob(self.decoder.rho_weight(genes_oi))

        fragmentexpression = (self.rho_bias[genes_oi] * torch.exp(rho_delta)) * self.libsize[cells_oi].unsqueeze(1)
        ...
        likelihood = likelihood_mixture.sum() * scale + likelihood_fragmentcounts.sum() * scale
    ...
    def parameters_dense(self):
        parameters = [parameter for module in self._modules.values() for parameter in (module.parameters_dense() if hasattr(module, "parameters_dense") else module.parameters())]
        def contains(x, y):
            return any([x is y_ for y_ in y])
        parameters.extend([p for p in self.parameters() if (not contains(p, self.parameters_sparse())) and (not contains(p, parameters))])
        return parameters
    ...
    def evaluate_pseudo(self, coordinates, latent = None, gene_oi = None, gene_ix = None):
        ...
        if gene_ix is None:
            ...
            local_gene_mapping = torch.zeros(genes_oi.max() + 1, device = device, dtype = torch.long)
            local_gene_mapping.index_add_(0, genes_oi, torch.arange(len(genes_oi), device = device))

            cut_local_gene_ix = local_gene_mapping[gene_ix]
            cut_local_cell_ix = torch.arange(latent.shape[0], device = cut_local_gene_ix.device)
            cut_local_cellxgene_ix = cut_local_cell_ix * len(genes_oi) + cut_local_gene_ix
        ...
        rho = torch.log((self.rho_bias[genes_oi][data.cut_local_gene_ix].exp() * rho_delta.exp()) * self.libsize.to(torch.float).mean())
        rho2 = (self.rho_bias[genes_oi][data.cut_local_gene_ix] * rho_delta) * self.libsize.to(torch.float).mean()

        return (
            self.track["likelihood_mixture"].detach().cpu(),
            rho_delta.detach().cpu(), rho.detach().cpu(),
    ...
    def rank(self, window, n_latent, device = "cuda"):
        ...
        for _, design_subset in tqdm.tqdm(design.groupby("batch")):
            ...
            pseudolatent = torch.nn.functional.one_hot(torch.from_numpy(design_subset["active_latent"].values).to(device), n_latent).to(torch.float)
            gene_ix = torch.from_numpy(design_subset["gene_ix"].values).to(device)
            
            mixture, rho_delta, rho, prob, prob2 = self.evaluate_pseudo(pseudocoordinates.to(device), latent = pseudolatent.to(device), gene_ix = gene_ix)
