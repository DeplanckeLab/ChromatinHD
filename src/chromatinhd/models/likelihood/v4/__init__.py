...
class Decoding(torch.nn.Module):
    ...
    def evaluate_pseudo2(self, coordinates, latent=None, gene_oi=None, gene_ix=None):
        device = coordinates.device
        if not torch.is_tensor(latent):
            if latent is None:
                latent = 0.0
    ...
    def rank2(self, window, n_latent, device="cuda", how="probs_diff_masked"):
        n_genes = self.rho_bias.shape[0]
        ...
        for _, design_subset in tqdm.tqdm(design.groupby("batch")):
            ...
            likelihood_mixture, rho_delta, rho, prob, prob2 = self.evaluate_pseudo2(
                pseudocoordinates.to(device),
                latent=pseudolatent.to(device),
                gene_ix=gene_ix,
            )
