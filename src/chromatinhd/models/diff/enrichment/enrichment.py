...
def count_motifs_genewise(relative_starts, relative_end, gene_ixs, motifscan, n_genes, window):
    motif_indices = []
    for relative_start, relative_end, gene_ix in zip(relative_starts, relative_end, gene_ixs):
        start_ix = gene_ix * (window[1] - window[0]) + relative_start
        end_ix = gene_ix * (window[1] - window[0]) + relative_end
        motif_indices.append(
            motifscan.indices[motifscan.indptr[start_ix] : motifscan.indptr[end_ix]] + gene_ix * motifscan.n_motifs
        )
    if len(motif_indices) > 0:
        motif_indices = np.hstack(motif_indices)
    else:
        motif_indices = np.array([], dtype=np.int64)
    motif_counts = np.bincount(motif_indices, minlength=motifscan.n_motifs * n_genes).reshape(
        (n_genes, motifscan.n_motifs)
    )
...
def select_background(
    ...
    # control
    # gc_back = count_gc(
    #     background_position_slices[:, 0], background_position_slices[:, 1], background_gene_ixs_slices,
    #       onehot_promoters
    # )
...
def enrich_windows(
    ...
    if n_background == 0:
        background_position_slices = np.stack([np.repeat(0, n_genes), np.repeat(window[1] - window[0], n_genes)]).T
