...
class FragmentEmbedder(torch.nn.Module):
    ...
    def __init__(self, n_genes, n_frequencies = 10, n_embedding_dimensions = 5, nonlinear = True, dropout_rate = 0., **kwargs):
        ...
        self.bias1 = EmbeddingTensor(n_genes, (self.n_embedding_dimensions-1, ), sparse = True)
        # self.bias1.weight.data.zero_()
        self.bias1.weight.data.uniform_()

        self.weight1 = EmbeddingTensor(n_genes, (self.sine_encoding.n_embedding_dimensions, self.n_embedding_dimensions-1), sparse = True)
    ...
    def forward(self, coordinates, gene_ix):
        ...
        embedding_learned = torch.einsum("ab,abc->ac", embedding, self.weight1(gene_ix)) + self.bias1(gene_ix)
        ...
        if self.dropout_rate > 0:
            embedding_learned = torch.nn.functional.dropout(embedding_learned, self.dropout_rate)

        embedding = torch.nn.functional.pad(embedding_learned, (0, 1), value = 1.)
...
class EmbeddingToExpression(torch.nn.Module):
    ...
    """
    def __init__(self, n_genes, mean_gene_expression, n_embedding_dimensions = 5, initialization = "ones", **kwargs):
