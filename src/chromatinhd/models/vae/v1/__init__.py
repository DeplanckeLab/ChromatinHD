...
def initialize_mixture(window, coordinates, genemapping, n_genes, n_components = 64):
    ...
    coordinates_binned = torch.from_numpy(np.digitize(coordinates.cpu().numpy(), bins)) - 1
    ...
    for gene_ix in range(n_genes):
        ...
        bincounts = torch.bincount(coordinates_binned[fragments_oi].flatten(), minlength = len(bins))
...
class Decoder(torch.nn.Module):
    def __init__(self, n_latent, n_genes, n_output_components, n_layers = 1, n_hidden_dimensions = 16):
        super().__init__()
        ...
        self.weight = EmbeddingTensor(n_genes, (n_hidden_dimensions if n_layers > 0 else n_latent, n_output_components), sparse = True)
    ...
    def forward(self, latent, genes_oi):
        ...
        nn_output = self.nn(latent).unsqueeze(1).unsqueeze(2) # broadcast across genes and across components
        print(weight.shape)
...
class Mixture(torch.nn.Module):
    ...
    def log_prob(self, value, weight_change, genes_oi, local_gene_ix):
        ...
        dist = torch.distributions.MixtureSameFamily(mixture_distribution, component_distribution, validate_args = True)
