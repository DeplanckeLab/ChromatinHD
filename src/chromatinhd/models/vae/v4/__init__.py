...
import peakfreeatac.models.positional.v17 as positional_model
import peakfreeatac.models.likelihood.v4 as likelihood_model
import peakfreeatac.models.likelihood.v4.spline
...
class VAE(torch.nn.Module):
    def __init__(
        ...
        transform = likelihood_model.spline.DifferentialQuadraticSplineStack(fragments.cut_coordinates, nbins = nbins, local_gene_ix = fragments.cut_local_gene_ix.cpu(), n_genes = fragments.n_genes)
    ...
    def forward_(
        ...
        cell_gene_embedding = self.embedding_gene_pooler.forward(fragment_embedding, local_cellxgene_ix, n_cells, n_genes)
        cell_embedding = self.cellgene_embedding_to_cell_embedding.forward(cell_gene_embedding, genes_oi)
        ...
        if self.baseline:
            ...
            mixture_delta_kl = mixture_delta_p.log_prob(self.decoder.logit_weight(genes_oi)) * self.n_total_cells / n_cells
