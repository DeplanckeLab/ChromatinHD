...
def initialize_mixture(window, coordinates, genemapping, n_genes, n_components=64):
    ...
    coordinates_binned = torch.clamp(
        torch.from_numpy(np.digitize(coordinates.cpu().numpy(), bins)) - 1, 0, len(bins)
    )
    ...
    for gene_ix in tqdm.tqdm(range(n_genes)):
        ...
        bincounts = torch.bincount(
            coordinates_binned[fragments_oi].flatten().to(torch.int),
            minlength=len(bins),
        )
...
def initialize_mixture(window, coordinates, genemapping, n_genes, n_components=64):
    ...
    bins = torch.arange(
        window[0] - binwidth / 2, window[1] - binwidth / 2 + 0.1, binwidth
    )
    ...
    bincounts = torch.bincount(
        coordinates_binned_flatten + genemapping_flatten * len(bins),
        minlength=len(bins) * n_genes,
    )
    ...
    logits = torch.log1p(
        bincount_reshaped[
            torch.repeat_interleave(torch.arange(n_genes), n_components),
            bins_chosen.flatten(),
        ].reshape((n_genes, n_components))
    )
...
class Mixture(torch.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("scale_lower_bound", torch.tensor(2.0) / self.ab)
    ...
    def log_prob(self, value, delta_logit, genes_oi, local_gene_ix):
        ...
        dist = torch.distributions.MixtureSameFamily(
            mixture_distribution, component_distribution, validate_args=False
        )
...
class VAE(torch.nn.Module):
    def __init__(
        ...
        locs, logits = initialize_mixture(
            fragments.window,
            fragments.coordinates,
            fragments.genemapping,
            fragments.n_genes,
    ...
    def forward(self, data, track_elbo=True):
        ...
        logit_change_cellxgene = logit_change.view(
            np.prod(logit_change.shape[:2]), logit_change.shape[-1]
        )
        logit_change_fragments = logit_change_cellxgene[data.local_cellxgene_ix]

        # fragment counts
        likelihood_left = self.mixture.log_prob(
            data.coordinates[:, 0], logit_change_fragments, genes_oi, data.local_gene_ix
        )
        likelihood_right = self.mixture.log_prob(
            data.coordinates[:, 1], logit_change_fragments, genes_oi, data.local_gene_ix
        )
...
class Decoding(torch.nn.Module):
    def __init__(
        self, fragments, cell_latent_space, n_components=32, decoder_n_layers=2
    ):
        super().__init__()
    ...
    def forward(self, data, latent=None, track_elbo=True):
        ...
        likelihood_left = self.mixture.log_prob(
            data.coordinates[:, 0], logit_change, genes_oi, data.local_gene_ix
        )
        likelihood_right = self.mixture.log_prob(
            data.coordinates[:, 1], logit_change, genes_oi, data.local_gene_ix
        )
