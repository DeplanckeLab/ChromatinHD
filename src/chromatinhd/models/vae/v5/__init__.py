...
import peakfreeatac.models.likelihood.v4 as likelihood_model
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, cut_coordinates, cut_local_cellxgene_ix, genes_oi, cells_oi):
        ...
        digitized = (torch.clamp_max(torch.floor(cut_coordinates * self.nbins), self.nbins-1).int() + (cut_local_cellxgene_ix.int() * self.nbins).int()).int()
        counts = torch.bincount(digitized, minlength = len(cells_oi) * len(genes_oi) * self.nbins)
        assert counts.shape == (len(cells_oi) * len(genes_oi) * self.nbins, ), (len(cells_oi) * len(genes_oi) * self.nbins, counts.shape)
        x = torch.log1p(counts.reshape((len(cells_oi), len(genes_oi) *self.nbins)).to(torch.float32))
...
class VAE(torch.nn.Module):
    rescale = True
    regularize_fragmentcounts = False
    fragments_scale = 1.
    mixtureautoscale = False
    mixturescale = 1.0
    mixturedistribution = "normal"
    def __init__(
        self,
        fragments,
        n_encoder_bins = 16,
        ...
        self.fragments_scale = fragments_scale

        self.regularize_fragmentcounts = regularize_fragmentcounts

        self.mixtureautoscale = mixtureautoscale
        if self.mixtureautoscale:
            self.mixture_p_scale = torch.nn.Parameter(torch.log(torch.tensor(1.0)))
        self.mixturescale = mixturescale
        self.mixturedistribution = mixturedistribution
    ...
    def forward_(
        ...
        # encode
        loc, scale = self.encoder(cut_coordinates, cut_local_cellxgene_ix, genes_oi, cells_oi)
        ...
        latent_kl = (latent_p.log_prob(latent) - latent_q.log_prob(latent))# * self.n_total_cells / n_cells
        ...
        if self.baseline:
            ...
            # mixture_delta_kl = torch.tensor(0., device = coordinates.device)
            if self.mixturedistribution == "normal":
                if self.mixtureautoscale:
                    mixture_delta_p = torch.distributions.Normal(0., torch.exp(self.mixture_p_scale))
                else:
                    mixture_delta_p = torch.distributions.Normal(0., self.mixturescale)
            else:
                mixture_delta_p = torch.distributions.Laplace(0., self.mixturescale)
            mixture_delta_kl = mixture_delta_p.log_prob(self.decoder.logit_weight(genes_oi))
            
            if self.rescale:
                mixture_delta_kl = mixture_delta_kl * self.n_total_cells / n_cells
        ...
        if self.regularize_fragmentcounts:
            rho_delta_p = torch.distributions.Normal(0., 1.0)
            rho_delta_kl = rho_delta_p.log_prob(self.decoder.rho_weight(genes_oi))
        else:
            rho_delta_kl = torch.tensor(0., device = coordinates.device)

        # likelihood
        likelihood = (
            likelihood_mixture.sum() * self.fragments_scale #* self.n_total_cuts / cut_coordinates.shape[0]
    ...
    def forward_likelihood_mixture(self, data):
        ...
        likelihood_mixture = self.track["likelihood_mixture"] = self.mixture.log_prob(data.cut_coordinates, data.genes_oi_torch, data.cut_local_gene_ix, mixture_delta)
    ...
    def parameters_dense(self):
        ...
        if self.mixtureautoscale:
            parameters.append(self.mixture_p_scale)
        return parameters
    ...
    def evaluate_latent(self, data):
        ...
        loc, scale = self.encoder(data.cut_coordinates, data.cut_local_cellxgene_ix, data.genes_oi_torch, data.cells_oi_torch)
