...
class VAE(torch.nn.Module):
    ...
    def forward_(
        ...
        latent_kl = latent_p.log_prob(latent) - latent_q.log_prob(
            latent
        )  # * self.n_total_cells / n_cells
            ...
            if self.mixturedistribution == "normal":
                if self.mixtureautoscale:
                    mixture_delta_p = torch.distributions.Normal(
                        0.0, torch.exp(self.mixture_p_scale)
                    )
                else:
                    mixture_delta_p = torch.distributions.Normal(0.0, self.mixturescale)
            else:
                mixture_delta_p = torch.distributions.Laplace(0.0, self.mixturescale)
        ...
        if self.regularize_fragmentcounts:
            rho_delta_p = torch.distributions.Normal(0.0, 1.0)
            rho_delta_kl = rho_delta_p.log_prob(self.decoder.rho_weight(genes_oi))
        else:
            rho_delta_kl = torch.tensor(0.0, device=coordinates.device)

        # likelihood
        likelihood = (
            likelihood_mixture.sum()
            * self.fragments_scale  # * self.n_total_cuts / cut_coordinates.shape[0]
