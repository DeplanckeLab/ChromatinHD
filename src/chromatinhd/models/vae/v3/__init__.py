...
class Decoder(torch.nn.Module):
    def __init__(self, n_latent, n_genes, n_spline_dimensions, n_layers = 1, n_hidden_dimensions = 32):
        super().__init__()
        ...
        self.spline_weight = EmbeddingTensor(n_genes, (n_hidden_dimensions if n_layers > 0 else n_latent, n_spline_dimensions), sparse = True)
        stdv = 1. / math.sqrt(self.spline_weight.weight.size(1)) / 100
    ...
    def forward(self, latent, genes_oi):
        ...
        spline = torch.matmul(nn_output.unsqueeze(1).unsqueeze(2), spline_weight).squeeze(-2)
...
class Mixture(torch.nn.Module):
    def __init__(self, n_bins, n_genes, window, loc_init = None, scale_init = None, logit_init = None, debug = False):
        super().__init__()
        self.n_bins = n_bins
        ...
        self.spline.weight.data[:, :] = 1. * default_init
...
class VAE(torch.nn.Module):
    def __init__(self, fragments, n_latent_dimensions = 10, n_bins = 100, decoder_n_layers = 2, n_frequencies = 50):
        super().__init__()
    ...
    def forward(self, data, track_elbo = True):
        ...
        likelihood_left = self.mixture.log_prob(data.coordinates[:, 0], spline_fragments, genes_oi, data.local_gene_ix)
        likelihood_right = self.mixture.log_prob(data.coordinates[:, 1], spline_fragments, genes_oi, data.local_gene_ix)
