...
def calculate_overlap(indices_A, indices_B):   
    overlap = np.in1d(convert(indices_A), convert(indices_B)).reshape(indices_A.shape[0], indices_A.shape[1])[:, 1:]
    return overlap
...
def score_overlap_phased(overlap, phases,  ks = (50, )):
    ...
    for k in ks:
        for phase, cells in phases.items():
            ...
            scores_cells.append(pd.DataFrame({
                "k":k,
                "overlap":jac_cells,
                "cell_ix":cells,
                "phase":phase
...
def score_ari_phased(clusters_B, indices_A, phases, ks = (50, )):
    ...
    for k in ks:
        ...
        for phase, cells in phases.items():            
            ari = sklearn.metrics.adjusted_rand_score(cluster_A[cells], cluster_B[cells])
            ami = sklearn.metrics.adjusted_mutual_info_score(cluster_A[cells], cluster_B[cells])
...
class Scorer:
    def __init__(self, models, transcriptome, folds, loaders, gene_ids, cell_ids, device="cuda"):
        ...
        sc.tl.leiden(transcriptome.adata, resolution = 1.)
    ...
    def score(self, loader_kwargs = None):
        ...
        for model_ix, (model, fold) in tqdm.tqdm(enumerate(zip(self.models, self.folds)), leave = False, total = len(self.models)):
            ...
            scores_overlap_, scores_overlap_cells_ = score_overlap_phased(overlap, phases, ks = self.ks)
            ...
            scores_woverlap_ = score_overlap_phased_weighted(overlap, phases, self.cell_weights, ks = self.ks)
        ...
        scores_overlap_cells_agg = scores_overlap_cells.groupby(["phase", "cell_ix"])[["overlap"]].mean()
