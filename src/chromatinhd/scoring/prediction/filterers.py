...
class WindowPairFilterer:
    def __init__(self, windows_oi):
            ...
            design.append(
                {
                    ...
                    "window_mid1": int(
                        window1.window_start
                        + (window1.window_end - window1.window_start) // 2
                    ),
                    "window_start2": window2.window_start,
                    "window_end2": window2.window_end,
                    "window_mid2": int(
                        window2.window_start
                        + (window2.window_end - window2.window_start) // 2
                    ),
                    "window1": window1_id,
                    "window2": window2_id,
                }
        ...
        design.index = (
            design["window_mid1"].astype(str) + "-" + design["window_mid2"].astype(str)
        )
    ...
    def filter(self, data):
        for design_ix, window_start1, window_end1, window_start2, window_end2 in zip(
            ...
            fragments_oi = select_window(
                data.coordinates, window_start1, window_end1
...
class MultiWindowFilterer:
    def __init__(self, window, window_sizes=(100, 200), relative_stride=0.5):
        ...
        design = pd.DataFrame(design)
        design.index = (
            design["window_start"].astype(str) + "-" + design["window_end"].astype(str)
        )
        design.index.name = "window"
        design["ix"] = np.arange(len(design))
        self.design = design
...
class WindowDirectionFilterer(WindowFilterer):
    def __init__(self, windows_oi, window, window_size=100):
        ...
        design = crossing(
            windows_oi, pd.DataFrame({"direction": ["forward", "reverse"]})
        )
    ...
    def filter(self, data):
        for design_ix, window_start, window_end, direction in zip(
            ...
            if direction == "forward":
                ...
                fragments_oi = ~(
                    (data.coordinates[:, 1] < window_end)
                    & (data.coordinates[:, 1] > window_start)
                )
            yield fragments_oi
